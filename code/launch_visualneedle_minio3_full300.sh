...
RUNNER="$PROJECT/run_visualneedle_minio3_full300.py"
SCORER="$PROJECT/score_visualneedle_full300.py"
...
E2M_PYTHON="$PYTHON" "$PYTHON" "$RUNNER" --merge --num-shards "$NUM_SHARDS" --command-string "$COMMAND" || exit 1
E2M_PYTHON="$PYTHON" "$PYTHON" "$SCORER" --output-dir "$OUTPUT"
